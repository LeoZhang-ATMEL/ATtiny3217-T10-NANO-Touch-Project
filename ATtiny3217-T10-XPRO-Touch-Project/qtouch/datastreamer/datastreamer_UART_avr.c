...
void datastreamer_output(void)
{
	...
	for (count_bytes_out = 0u; count_bytes_out < DEF_NUM_SENSORS; count_bytes_out++) {
		...
		uint16_t temp_data = touch_key_node_mapping_4p[count_bytes_out] >> 2;
#if (DEF_PTC_CAL_OPTION == CAL_AUTO_TUNE_CSD)
		/* CSD */
		u8temp_output = qtlib_acq_set1.qtm_acq_node_config[temp_data].node_csd;
		datastreamer_transmit(u8temp_output);
#else
		/* Prescalar */
		u8temp_output = NODE_PRSC(qtlib_acq_set1.qtm_acq_node_config[temp_data].node_rsel_prsc);
		datastreamer_transmit(u8temp_output);
#endif
#endif
